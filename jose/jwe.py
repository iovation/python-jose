...
def encrypt(plaintext, key, enc=ALGORITHMS.A256GCM, alg=ALGORITHMS.DIR,
            zip=None, cty=None, kid=None):
    ...
    Args:
        ...
        enc (str, optional): The content encryption algorithm used to
        ...
        alg (str, optional): The cryptographic algorithm used
    ...
    encoded_header = _encoded_header(alg, enc, zip, cty, kid)

    try:
        plaintext = _compress(zip, plaintext)
        enc_cek, iv, cipher_text, auth_tag = _encrypt_and_auth(
            key, alg, enc, zip, plaintext, encoded_header)
...
def _get_cek(enc, alg, key):
    ...
    """
    if alg == ALGORITHMS.DIR:
        cek, wrapped_cek = _get_direct_key_wrap_cek(enc, key)
    elif alg in ALGORITHMS.RSA:
        cek, wrapped_cek = _get_rsa_key_wrap_cek(enc, key)
    else:
...
def _get_direct_key_wrap_cek(enc, key):
    """
...
def _get_rsa_key_wrap_cek(enc, key):
    ...
    """
    cek_bytes = _get_random_cek_bytes_for_enc(enc)
    _, wrapped_cek, _ = key.encrypt(cek_bytes)
    return cek_bytes, wrapped_cek
