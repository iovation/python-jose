...
@pytest.mark.parametrize("algorithm", RSAES_ALGOS)
@pytest.mark.skipif(
    None in (default_backend, pyca_rsa, PyCryptoRSAKey),
    reason="PyCrypto backend not available"
)
def test_pycrypto_encrypt_unencoded_cleartext(private_key_pem, algorithm):
    private_key = PyCryptoRSAKey(private_key_pem, algorithm)
    msg = b'test'
